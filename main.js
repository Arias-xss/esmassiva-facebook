...
const userDataDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "puppeteer_profile_")
);
...
async function loginUser(targetPage) {
  ...
  await puppeteer.Locator.race([
    ...
    targetPage.locator(
      '::-p-xpath(//*[@data-testid=\\"royal_login_button\\"])'
    ),
...
async function checkMessages(targetPage, controlandoTimes) {
  ...
  if (
    await targetPage.evaluate(
      () => document.querySelector('[aria-label="Cerrar"]') !== null
    )
  ) {
    if (
      await targetPage.evaluate(
        () =>
          document.querySelector(
            '[aria-label="¿Continuar sin sincronizar?"] > div:nth-of-type(3) > div > div > div:nth-of-type(2)'
          ) !== null
      )
    ) {
  ...
  for (const index of [
    ...
    if (
      ...
      console.log(
        `Mensaje leido? ${
          selectorMessage != "600" ? "Si" : "No"
        } - POS ${index}`
      );

      // Responder a mensajes no leidos
      if (selectorMessage == "600") {
        ...
        const [
          ...
          chatOpennedValue,
          secondResponseValue,
        ] = await targetPage.evaluate((phoneNumber) => {
          ...
          if (
            ...
            const extractAndNormalizePhoneNumber = (
              message,
              phone = phoneNumber
            ) => {
            ...
            for (const chatItem of result.singleNodeValue.childNodes[0]
              ...
              customerPhoneNumber =
                extractAndNormalizePhoneNumber(chatItem.innerText) !== false
                  ? extractAndNormalizePhoneNumber(chatItem.innerText)
                  : customerPhoneNumber;
              chatOpenned =
                chatOpenned === false
                  ? chatItem.innerText.search(
                      "Cada pedido es procesado  por Whatsapp"
                    ) > -1
                  : chatOpenned;
              secondResponse =
                secondResponse === false
                  ? chatItem.innerText.search(
                      "si podrias brindarme tu número de celular asi puedo escribirte"
                    ) > -1
                  : secondResponse;
            }

            const customerData = result.singleNodeValue
              .getAttribute("aria-label")
              .split("·");
            customerData[0] = customerData[0]
              .replace("Mensajes de la conversación con el título ", "")
              .trim();
        ...
        if (customerPhoneNumberValue) {
          const records = [
            {
              cliente: customerDataValue[0],
              producto: customerDataValue[1],
              numero: customerPhoneNumberValue,
              cuenta: `0${phoneNumber}`,
            },
          ];
        ...
        if (!chatOpennedValue && !secondResponseValue) {
          await puppeteer.Locator.race([
            ...
            `Buenas. Si. 😄 Cada pedido es procesado  por Whatsapp ✅ Podes escribirme al Whatsapp 0${phoneNumber} O directo en el link https://wa.me/595${phoneNumber}?text=${encodeURIComponent(
              "Buenas!"
            )} 😊 \n`
          );
          ...
          await puppeteer.Locator.race([
            ...
            `Para una mejor atención, si podrias brindarme tu número de celular asi puedo escribirte, gracias 😊 \n`
          );
        }
      ...
      if (!fs.existsSync(cookiesFilePath)) {
        ...
        if (currentPage.search("/two_step_verification/authentication/") > -1) {
          if (
            ...
            const captchaUrl = await targetPage.evaluate(() =>
            ...
            await puppeteer.Locator.race([
              targetPage.locator(
                ".x1ja2u2z.x78zum5.x2lah0s.x1n2onr6.xl56j7k.x6s0dn4.xozqiw3.x1q0g3np.x972fbf.xcfux6l.x1qhh985.xm0m39n.x9f619.xtvsq51.xi112ho.x17zwfj4.x585lrc.x1403ito.x1fq8qgq.x1ghtduv.x1oktzhs"
              ),
            ])
              .setTimeout(timeout)
              .click({
...

    {
      ...
      if (
        ...
        if (
          ...
          await puppeteer.Locator.race([
            targetPage.locator(
              "div.x9f619 > div.x2lah0s div:nth-of-type(2) > div > div:nth-of-type(1) > div > div > div > div > div > div > div > div > div > div > div.x1iyjqo2 > div > div > div > div > div:nth-of-type(2) div"
            ),
            targetPage.locator(
              '::-p-xpath(//*[@id=\\":r23:\\"]/div/div/div/div/div/div[2]/div/div[1]/div/div/div/div/div/div/div/div/div/div/div[2]/div/div/div/div/div[2]/span/span/div)'
            ),
            targetPage.locator(
              ":scope >>> div.x9f619 > div.x2lah0s div:nth-of-type(2) > div > div:nth-of-type(1) > div > div > div > div > div > div > div > div > div > div > div.x1iyjqo2 > div > div > div > div > div:nth-of-type(2) div"
            ),
          ])
            .setTimeout(timeout)
            .click({
